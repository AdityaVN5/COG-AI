...
│                                                             │
│  ┌─────────────────┐  ┌──────────────────┐  ┌───────────┐  │
│  │  Graph Canvas   │  │  Chat Interface  │  │ Analysis  │  │
│  │  (react-zoom-   │  │  (SSE Streaming) │  │  View     │  │
│  │   pan-pinch)    │  │                  │  │           │  │
│  └────────┬────────┘  └────────┬─────────┘  └─────┬─────┘  │
...
│                                                             │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────────────┐  │
│  │ graph.py    │  │  chat.py     │  │ main.py endpoints  │  │
│  │             │  │              │  │                    │  │
│  │ networkx    │  │ Stage 1:     │  │ /api/graph         │  │
│  │ DiGraph     │  │ Schema →     │  │ /api/chat          │  │
│  │ community   │  │ SQL (LLM)    │  │ /api/history       │  │
│  │ detection   │  │              │  │ /api/analysis/     │  │
│  │             │  │ Stage 2:     │  │   broken-flows     │  │
│  │ spring_     │  │ Data →       │  │                    │  │
│  │ layout      │  │ Answer (LLM) │  │                    │  │
│  └──────┬──────┘  └──────┬───────┘  └────────────────────┘  │
│         │                │                                   │
...
│  │  outbound_deliveries  │  journal_entry_items         │   │
...
│         SQL Generation  ·  Answer Synthesis  ·  Streaming    │
└──────────────────────────────────────────────────────────────┘
```
...
|---|---|---|
| Data Ingestion | `backend/database.py` | Reads `.jsonl` files, normalizes nested JSON, writes to SQLite |
| Graph Builder | `backend/graph.py` | Constructs `networkx.DiGraph`, assigns layout coords, detects clusters |
| Chat Engine | `backend/chat.py` | Two-stage LLM pipeline: SQL generation with history context + answer synthesis |
| API Server | `backend/main.py` | FastAPI routes, graph caching, history persistence, broken flow detection |
| Graph UI | `src/App.tsx` | Interactive canvas, draggable nodes, SVG edges, chat panel |
| Analysis UI | `src/components/AnalysisView.tsx` | Broken flow dashboard (Stuck Orders, Unbilled Deliveries, Unpaid Invoices) |
| History UI | `src/components/HistoryView.tsx` | Past conversation browser with delete capability |
